# Exact-artifact `uni-7` integration evidence

`uni7_capture.py` is a chain-capable evidence harness for the ten required
...
python3 integration/uni7_capture.py \
  ...
  --transcript /secure-work/multi-fund.json \
...
python3 integration/uni7_capture.py \
  --config deployment/environments/uni-7.json \
  --state /secure-work/juno-voice/uni-7/state.json \
  --transcript /secure-work/guardian.json \
