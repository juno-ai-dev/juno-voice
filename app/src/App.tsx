...
export function App({ source: supplied, registrySource: suppliedRegistry, config, registryTransactionFlow, walletAddress }: { source?: VoiceDataSource; registrySource?: RegistryDataSource; config: AppConfig; registryTransactionFlow?: RegistryTransactionFlow; walletAddress?: string }) {
  ...
  return <div className="shell juno-grid">
    ...
    {view === "projects" ? <Suspense fallback={<main><p>Loading project registry…</p></main>}><Registry source={registrySource} config={config} transactionFlow={registryTransactionFlow} sender={walletAddress} /></Suspense> : <Ledger source={source} config={config} />}
