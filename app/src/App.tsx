...
import { PublicTransactions, WalletConnection } from './wallet';
import { WalletButton } from './WalletActions';
...
export function App({ source: suppliedSource, config: suppliedConfig }: { source?: VoiceDataSource; config?: AppConfig }) {
  ...
  useEffect(() => wallet.onChange(() => setAccount(null)), [wallet]);
  ...
  return <div className="shell grid"><header className="topbar"><div className="topbar-inner"><a className="brand" href="/" onClick={(event) => { event.preventDefault(); navigate(null); }}><img src="/assets/logo-salmon.svg" alt="" /><img src="/assets/wordmark-salmon.svg" alt="Juno" /><span>VOICE</span></a><nav aria-label="Primary"><a href="/" aria-current={requestId === null ? 'page' : undefined} onClick={(event) => { event.preventDefault(); navigate(null); }}>Signal</a><span className="testnet">TESTNET · UNI-7</span><WalletButton wallet={wallet} account={account} onAccount={setAccount}/></nav></div></header>{requestId === null ? <Ledger key={refresh} source={source} config={config} account={account} transactions={transactions} onSuccess={() => setRefresh(value => value + 1)} onOpen={(id) => navigate(id)} /> : <Detail key={`${requestId}-${refresh}`} source={source} config={config} id={requestId} account={account} transactions={transactions} onSuccess={() => setRefresh(value => value + 1)} onBack={() => navigate(null)} />}<footer><p>Direct-RPC reading works without a wallet · Public submission and eligible refund signing only · Voting safety-gated</p><p>Design assets and tokens © 2026 Jake Hartnell, MIT · Juno Design System <a href="https://github.com/juno-ai-dev/juno-design-system/commit/0dc0ae9">0dc0ae9</a></p></footer></div>;
}
