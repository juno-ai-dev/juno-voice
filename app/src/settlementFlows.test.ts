...
describe("settlement intent actor, state, schema, and time boundaries", () => {
  it("builds the exact creator nomination with recipient/evidence/digest/rationale and no funds", () => {
    expect(nominatePayoutIntent(open(), creator, { recipient, evidenceUri: "ipfs://evidence", evidenceDigest: digest, rationale: "Shipped" }, config.contract))
  ...
  it.each([
    ["wrong actor", (s: BountyDetail) => nominatePayoutIntent(s, other, { recipient, evidenceUri: "ipfs://evidence", evidenceDigest: digest, rationale: "x" }, config.contract)],
    ["paused", (s: BountyDetail) => nominatePayoutIntent({ ...s, pause: { ...s.pause, paused: true } }, creator, { recipient, evidenceUri: "ipfs://evidence", evidenceDigest: digest, rationale: "x" }, config.contract)],
    ["expired equality", (s: BountyDetail) => nominatePayoutIntent({ ...s, chainTimeNanos: s.bounty.expires_at }, creator, { recipient, evidenceUri: "ipfs://evidence", evidenceDigest: digest, rationale: "x" }, config.contract)],
    ["round limit", (s: BountyDetail) => nominatePayoutIntent({ ...s, bounty: { ...s.bounty, next_round: s.bounty.terms.max_rounds + 1 } }, creator, { recipient, evidenceUri: "ipfs://evidence", evidenceDigest: digest, rationale: "x" }, config.contract)],
  ...
  it("enforces sole actor, exact round, and strict close/expiry deadlines", () => {
    ...
    expect(confirmSolePayoutIntent(sole, creator, 1, config.contract).executeMessage).toEqual({ confirm_sole_payout: { bounty_id: 1, round: 1 } });
    expect(declineSolePayoutIntent(sole, creator, 1, "Not complete", config.contract).executeMessage)
      .toEqual({ decline_sole_payout: { bounty_id: 1, round: 1, reason: "Not complete" } });
    expect(() => confirmSolePayoutIntent(sole, other, 1, config.contract)).toThrow(/contributor/);
    expect(() => confirmSolePayoutIntent(sole, creator, 2, config.contract)).toThrow(/Wrong payout round/);
    expect(() => confirmSolePayoutIntent({ ...sole, chainTimeNanos: round.closes_at! }, creator, 1, config.contract)).toThrow(/closed/);
    expect(() => declineSolePayoutIntent({ ...sole, chainTimeNanos: sole.bounty.expires_at }, creator, 1, "x", config.contract)).toThrow(/closed/);
  });
  ...
  it("uses immutable snapshot weight, permits revisions, and does not substitute expiry for the multi-vote close", () => {
    ...
    const intent = votePayoutIntent(revised, creator, 1, "yes", "Changed after review", config.contract);
    ...
    expect(() => votePayoutIntent({ ...base, chainTimeNanos: base.activeRound!.closes_at! }, creator, 1, "no", "", config.contract)).toThrow(/closed/);
    expect(() => votePayoutIntent(base, toBech32("juno", new Uint8Array(20).fill(9)), 1, "yes", "", config.contract)).toThrow(/snapshotted/);
  });
  ...
  it.each([
    ...
    expect(finalizePayoutIntent(state, 1, config.contract).consequences[0]).toMatch(expected);
    expect(finalizePayoutIntent(state, 1, config.contract).funds).toEqual([]);
    expect(() => finalizePayoutIntent({ ...state, chainTimeNanos: (BigInt(round.closes_at!) - 1n).toString() }, 1, config.contract)).toThrow(/still open/);
  });

  it("supports public expiry at equality, scoped cancellation, and exactly-once refunds", () => {
    const base = open();
    expect(expireBountyIntent({ ...base, chainTimeNanos: base.bounty.expires_at }, config.contract).executeMessage).toEqual({ expire: { bounty_id: 1 } });
    expect(() => expireBountyIntent({ ...base, chainTimeNanos: (BigInt(base.bounty.expires_at) - 1n).toString() }, config.contract)).toThrow(/not expired/);
    expect(cancelSoleFundedIntent(base, creator, "No longer needed", config.contract).executeMessage)
      .toEqual({ cancel_sole_funded: { bounty_id: 1, reason: "No longer needed" } });
    expect(() => cancelSoleFundedIntent(base, other, "x", config.contract)).toThrow(/creator/);
    const refundable = { ...base, bounty: { ...base.bounty, status: "refunding" as const, refund_reason: "expired" as const } };
    expect(claimRefundIntent(refundable, creator, config.contract).executeMessage).toEqual({ claim_refund: { bounty_id: 1 } });
    expect(() => claimRefundIntent({ ...refundable, claims: [{ bounty_id: 1, contributor: creator, amount: "2500000", claimed_at: "2" }] }, creator, config.contract))
