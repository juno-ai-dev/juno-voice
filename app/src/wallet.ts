...
function isDefinitelyPrebroadcast(error:unknown):boolean{const message=error instanceof Error?error.message:String(error);return /^(?:request|user) (?:rejected|denied|cancelled|canceled)(?:[.:]|$)/i.test(message)||/\b(?:wallet\s+)?locked\b|wrong chain|chain.*mismatch|unsupported chain|insufficient funds/i.test(message)}
