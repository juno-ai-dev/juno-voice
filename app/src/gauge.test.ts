...
const rawGauge = { id: 0, title: "Hack Juno", adapter: config.registryContract, epoch_size: 1000, min_percent_selected: "0.01", max_options_selected: 10, max_available_percentage: "0.5", is_stopped: false, next_epoch: 3000, reset: null, snapshot_policy: { min_turnout_bps: 2000, epoch_budget: "10000000", denom: "ujuno", retained_option: "do-not-distribute", execution_window_seconds: 1000 }, current_epoch: 2 };
const ballot = { ballot: { voter, power: "100", votes: [{ option: "project:1", weight: "0.5" }], cast_at: 2100, revised_at: 2200, revisions: 1, receipt_index: 1 } };
const registryProject = { id: 1, owner: voter, payout_address: voter, metadata_uri: "ipfs://alpha-project", metadata_digest: `sha256:${"a".repeat(64)}`, status: "active", created_at: "1", updated_at: "1", status_history_count: 1, address_history_count: 0, provenance: { bonded_registration: { applicant: voter } }, bond: { amount: "1000000", depositor: voter, state: "deposited" }, pending_payout_address: null, latest_review: null };
...
function connector(allocationCount = 2) {
  ...
  return vi.fn(async () => ({
    ...
    queryContractSmart: async (address: string, query: Record<string, Record<string, unknown>>) => {
      ...
      if (address === config.registryContract) return action === "pause" ? { admissions_stopped: false, adapter_stopped: false, reason: null, actor: null, changed_at: null } : action === "all_options" ? { options: ["do-not-distribute", "project:1"] } : { projects: [registryProject] };
      ...
      if (action === "epoch_allocations") {
        ...
        const values = epoch === 1 ? [["do-not-distribute", "0"], ["project:1", "400"]] : [["do-not-distribute", "100"], ["project:1", "300"]];
...
describe("strict pinned gauge schema decoders", () => {
  it("decodes exact gauge, epoch, ballot and historical power shapes", () => {
    ...
    expect(mapBallotResponse(ballot)).toMatchObject({ voter, votes: [{ option: "project:1", weight: "0.5" }] });
...
describe("canonical epoch and connected-ballot discovery", () => {
  it("verifies every deployment binding and discovers current/previous fixed snapshots", async () => {
    ...
    expect(data.options.map((item) => item.option)).toEqual(["do-not-distribute", "project:1"]);
    expect(data.previousOptions[1]).toEqual({ option: "project:1", power: "400" });
