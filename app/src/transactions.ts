...
const PAYABLE_ACTIONS = new Set(["create_bounty", "contribute"]);
function validateIntent(intent: TransactionIntent, authorization: WalletAuthorization): void {
  ...
  if (PAYABLE_ACTIONS.has(action)
    ? intent.funds.length !== 1 || !exactJunoCoin(intent.funds[0])
    : intent.funds.length !== 0)
    throw new TransactionSafetyError("invalid_transaction", "Attached funds do not match the approved action policy.");
