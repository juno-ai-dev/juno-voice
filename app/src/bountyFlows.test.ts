...
describe("schema-backed bounty execute construction", () => {
  ...
  it("constructs exact contribute and preserves amounts as strings", () => {
    expect(contributeIntent({ ...bounty, terms: { ...bounty.terms, max_bounty_total: "340282366920938463463374607431768211455" } },
      "9007199254.740993", bounty.creator, [bounty.creator], state)).toMatchObject({
  ...
  it("uses chain time and snapshotted bounty caps for contribution eligibility", () => {
    expect(() => contributeIntent({ ...bounty, expires_at: state.chainTimeNanos }, "1", null, [], state)).toThrow(/chain time/);
    expect(() => contributeIntent({ ...bounty, total_contribution: bounty.terms.max_bounty_total }, "1", null, [], state)).toThrow(/snapshotted/);
    expect(() => contributeIntent({ ...bounty, contributor_count: bounty.terms.max_contributors }, "1", "juno1new", [], state)).toThrow(/contributor cap/);
